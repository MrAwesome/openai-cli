...
export default class OpenAICompletionCommand extends OpenAICommand<OpenAICompletionCLIOptions> {
    ...
    static addSubCommandTo(program: commander.Command, scriptContext: ScriptContext): commander.Command {
        const basicCommand = program
            .command(this.subCommandName)
            .description(this.description);
        const fullcommand = openaiCompletionCLIParser(basicCommand, scriptContext);

        return fullcommand;
    }

    verifyCLI(): OpenAICompletionCLIOptions | VerifyCLIError {
        const {unverifiedSubCommandOpts: camelCasePreVerifiedOpts, subCommandArgs} =
            this.ctx;
...
function openaiCompletionCLIParser(subCommand: commander.Command, scriptContext: ScriptContext): commander.Command {
    const {
        frequency_penalty,
        prompt_flag,
        max_tokens,
    ...
    if (scriptContext.isRemote === false) {
        cmd.option('-f, --prompt-file <prompt_file>', `A file whose contents should be read and used as the OpenAI completion prompt. Mutually exclusive with --prompt or passing args as the prompt.`, prompt_file)
    }
