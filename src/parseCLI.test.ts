...
import {ScriptContext, SubCommand, SubCommandContext} from "./types";
...
describe("testCLIParsing", () => {
    const data: [string, ScriptContext, CheckType][] = [
        ...
        [...sc("remote", ["--file", "anything.txt"]), {commanderErrorCode: "commander.unknownOption"}],
        [...sc("remote", ["-f", "anything.json"]), {commanderErrorCode: "commander.unknownOption"}],
        [...sc("remote", ["openai-completion", "--prompt-file", "anything.txt"]), {commanderErrorCode: "commander.unknownOption"}],
        [...sc("remote", ["openai-completion", "-f", "anything.json"]), {commanderErrorCode: "commander.unknownOption"}],
    ];
    ...
    test.each(data)(
        ...
        (title, scriptContext, checkType) => {
            ...
            if (typeof checkType === "object" && "validator" in checkType) {
                try {
                    const res = parseCLI(scriptContext, COMMANDER_OUTPUT_CONTROL(title));
